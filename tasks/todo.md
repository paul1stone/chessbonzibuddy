...
- [ ] Build waves:
  ...
  - [ ] In flight: Task 4 0bb4047 + Task 5b 01cb722 landed (reviews running); Task 2 c6d0702 landed (transform check PASS, review dispatched; board start-alias crash fixed by fixer); Task 6 7bc244a landed (review dispatched); Task 5a 97be797 landed (ReviewStatusBar separate export; review dispatched); Task 9 PASS (fixer: generic demo rows, inert read-only boards; fixture-dup + still-frame notes logged); TASK 7 CLOSED (combined re-review 6a745a2+b55a7ae PASS zero defects); Task 10 review FAIL (SSR cascade leak, mobile pre-hydration overflow) → fixed in the streamline-layout commit (CSS custom-prop breakpoints); hero path verified clean; TASK 11 CLOSED 65c5f80 + addenda 4f55cbc (stale comment reworded, reply budget 30s, reply measured 6.1s): 19/19 e2e (Bonzi assertion green post-fix), gates pass, / budget 186.8KB (under part 1), Lighthouse / 82 perf (WebGL variance, recorded) 96 a11y, /app 97/96. Em-dash sweep done (4 log strings + 3 toasts). Bulk toast SHIPPED b55a7ae (verified 404/transport/success) (deps 0/3/8 all closed, files disjoint from wave 3)
  - [ ] Reviews: Task 4 CLOSED (fixer f321e94: visible tick + aria-pressed, press-shift restored via ! utilities, APG tablist nav); Task 5b PASS (cursor-suffix fixed by fixer; swatch-hex defect REJECTED — piece palette is plan-sanctioned via eval-bar; contrast question logged for final review); Task 2 CLOSED (fixer 7d54d7d: all four minors, cycle → DAG); Task 6 CLOSED (re-review PASS incl. 8831e3d, chip measured 51x23); Task 5a FAIL→fixed by fixer (reserve 117, :empty statusbar rule, eval-chart string-index click bug — pre-existing, now works); tabs-roving a11y delta logged for final review; CLOSED. ENGINE RESOLVED (two independent bugs, distinct scenarios): 42ed93e init race = THE user-visible brick (move within ~3.5s of opening play, silent forever — reason from this for any loading-gate UX); 60a172d 30s cap = long-TC failure (30+0 always aborted at 30.0s) + hardening. Task 11 correction folded in. Task 11's red Bonzi-reply e2e awaiting re-run. Original note: 60a172d: root cause was evaluateWithClock's 30s safety cap below Stockfish's clock budget (always fatal at 30+0; ~10s headroom at default 5+0), NOT an init hang; 'hang' repros were worktrees missing the gitignored 113MB wasm. Fix + fail-fast onerror + quit settles waits + stop on timeout + loud slow-init in worktree harnesses at BOTH 7bc244a and parent — conflicts with implementer's live harness (engine answered e5); Task 7's real-app verification must confirm Bonzi replies; if it fails there, suspect the upstream engine.ts rework, not wave 3; implementers 5a/6 self-audited cascade fixes e88d005/8831e3d routed to their reviewers
  - [ ] Then: Task 10 (after 9), Task 11, final review
- [x] Final Fable review: FIX → all fixes applied (overflow clamp + 1024 e2e; import copy; queue purge + benign 404; Start menu Practice/GitHub; delete toast; focus-to-Start; thinking-before-await polish). Loading gate: NOT needed (reviewer, reasoned from the race). Suite gaps + nits → follow-ups
...
- Suite gaps (manually verified, untested): useBoardSize unit; 375 taskbar-switching e2e; practice-demo move-play e2e; play-boot console-error collection
- Nits deferred: cascade clamp on open(); will-change during drag; eval-chart --chart-* tokens; worstLossIndex fallback if fixture regenerated; practice-demo source-game credit; recent-games gif-to-skeleton; title-bar/tabpanel focus ring strength
...
- Bonzi thinks ~70s/move at 30+0 (correct clock behavior, now unblocked) — product decision: cap think time for the mascot game? (user history says 'sf move fast')
- public/stockfish is gitignored; fresh worktrees ship a dead engine until npm install (worktree footgun)
- Bulk-import toast volume scales with batch size (consider one summary toast for large batches)
- Open-but-idle play window silently holds the analysis queue (plan-sanctioned; consider a paused-while-playing hint)
- RecentGames greys rows optimistically even when an import fails (needs onImport to become awaitable with per-game outcomes — interface change)
