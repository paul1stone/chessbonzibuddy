...
- [ ] Build waves:
  ...
  - [ ] In flight: Task 4 0bb4047 + Task 5b 01cb722 landed (reviews running); Task 2 c6d0702 landed (transform check PASS, review dispatched; board start-alias crash fixed by fixer); Task 6 7bc244a landed (review dispatched); Task 5a 97be797 landed (ReviewStatusBar separate export; review dispatched); Task 9 PASS (fixer: generic demo rows, inert read-only boards; fixture-dup + still-frame notes logged); TASK 7 6a745a2 LANDED (full verification incl. live Bonzi replies + queue-guard case; review dispatched); Task 10 86cc3c9 LANDED (no-jump + GSAP composition proven; review dispatched); TASK 11 DISPATCHED with Bonzi-reply e2e assertion to settle the engine question. Approved follow-up: bulk-import HTTP-error toast (pre-existing silent drop; optimistic grey-out UX left as known issue) (deps 0/3/8 all closed, files disjoint from wave 3)
