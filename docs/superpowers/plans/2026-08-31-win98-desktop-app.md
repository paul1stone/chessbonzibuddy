...
- [ ] **Step 7: Verify.** `npm run test` (new demo-utils tests), landing page: demos autoplay in view, scrub takes over; for practice, read `worstLossIndex()`'s move from the generated fixture and verify in a harness that dropping that move's own `bestMove` reports correct (expected SAN is fixture-determined, never hardcoded). Reduced motion: static states. Landing initial-bundle check: `curl` the built page — no chess.js/react-chessboard in initial chunks (grep for "Chessboard"). Part-1 e2e 9/9.
