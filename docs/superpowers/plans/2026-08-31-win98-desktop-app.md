...
- Produces: `GamesWindow()`, `ProfileWindow()`, `ImportWindow({ onImportUrl, onBulkImport, importing }: { onImportUrl: (url: string) => Promise<void>; onBulkImport: (games: RecentGameData[]) => Promise<void>; importing: boolean })` — the SSE/queue logic stays in the page (Task 7), which passes these down. `RecentGameData` type moves to `src/lib/chess-com.ts`? No — it stays exported from `recent-games.tsx` and `import-window.tsx` re-exports it.
...
- [ ] **Step 1: New `page.tsx`.** Keeps VERBATIM: `runAnalysis` SSE loop (L330-394), queue processor (L397-423), `handleSubmit`/`handleBulkImport` import handlers (L430-505, minus view side-effects — `setActiveGame` in `game-store` still flips `view`, which is now inert). Removes: LoginScreen, ImportView, the three top bars, all view branching. Adds `DEFS: Record<WindowId, WindowDef>` wiring each window body (`games` → `<GamesWindow/>`, `import` → `<ImportWindow onImportUrl={...} onBulkImport={...} importing={isLoading}/>`, `review` → `<ReviewWindow .../>` with title `` `${activeGame.whitePlayer} vs ${activeGame.blackPlayer}` `` (fallback "Game review"), `practice` → `<PracticeWindow/>`, `play` → `<PlayWindow/>`, `profile` → `<ProfileWindow/>`). On-mount effect: if a username is linked open `games`, else open `profile` (guard with a ref so it runs once; skip both when `?view=play-bonzi`).
- [ ] **Step 2: `view-param-sync.tsx`:** `setView("play-bonzi")` → `useWindowStore.getState().open("play")`.
- [ ] **Step 3: `(app)/layout.tsx`:** wrapper class `dark ... [color-scheme:dark]` → `retro app ${msSans.variable} ${vt323.variable}`; drop Geist imports, import `msSans, vt323` from `@/fonts/retro-fonts`; remove `<DashboardLayout>` (children render bare); Toaster gets `position="bottom-right"` + `offset={{ bottom: 38 }}` and `richColors` removed.
...
- [ ] **Step 6: Full verification.** Dev server: /app cold → desktop, teal, icons, profile or games window opens; link account → games list loads; import a URL → toast, review window opens with analysis progressing in the status bar (SSE live-scrub visible); practice opens from review; play boots Stockfish; mobile 375px single-window flow; `/` unchanged. `npm run typecheck && npx eslint src --quiet` (baseline: board.tsx only) `&& npm run test && npm run build` (route table unchanged).
...
**Files:**
- Create: `scripts/generate-demo-analysis.mjs`
- Create: `src/components/landing/demo/opera-game.json` (generated, committed)
- Modify: `package.json` scripts (`"demo-fixture": "node scripts/generate-demo-analysis.mjs"`)
...
**Files:**
- Create: `src/components/landing/demo/review-demo.tsx`, `practice-demo.tsx`, `import-demo.tsx`, `demo-utils.ts`, `demo-utils.test.ts`
...
- Produces: `demo-utils.ts` exports `DEMO = { pgn, analysis }` (typed import of the JSON), `demoPositions(): string[]` (FENs via chess.js), `worstLossIndex(): number` (index into `analysis.moves` of the black-side mistake/blunder with max win% swing; tested).
...
- [ ] **Step 6: Poster manifest swap** per Files above; e2e "unmounts the poster" test must still pass (poster.json `hero: true` committed).
- [ ] **Step 7: Verify.** `npm run test` (new demo-utils tests), landing page: demos autoplay in view, scrub takes over, practice accepts `Qb8+`? — the worst black loss in the Opera Game fixture will be a specific move; assert in the harness that dropping the fixture's own `bestMove` reports correct. Reduced motion: static states. Landing initial-bundle check: `curl` the built page — no chess.js/react-chessboard in initial chunks (grep for "Chessboard"). Part-1 e2e 9/9.
...
- [ ] **Step 1: `window-stack.tsx`** becomes client (`"use client"`): each item gets local `{ dx, dy, z }` state; title bar `onPointerDown` from `useDrag` (disabled on mobile/reduced-motion) adds `translate` beyond the cascade offset and bumps a local z counter; the title bar shows `cursor: default` and `touch-action: none` only on md+. First drag adds nothing else — windows remain in flow (`position: relative`).
...
(`[data-column]` is react-chessboard's column marker; verify the selector against the DOM during implementation and adjust with a comment.)
...
| 2 | Task 1, Task 3, Task 8 | Disjoint: store/hook; chess plumbing; fixture script. (Task 3 consumes only Task 1's exported hook TYPE — serialize 3 after 1 within the wave by having Task 3 start when Task 1's commit lands; if racing, Task 3 may stub `useWindowFocused` import until then. Simpler: run Task 1 first, then 3 and 8 truly parallel — treat as wave 2a/2b.) |
| 3 | Task 2, Task 4, Task 5, Task 6 | Chrome (desktop/*); windows A (windows/{games,profile,import} + import/*); windows B (review/* practice/* move-controls + windows/{review,practice}); windows C (play/* + windows/play). All file-disjoint. |
| 4 | Task 7, Task 9 | Page composition (app tree); homepage demos (landing tree). Disjoint. |
| 5 | Task 10 | Touches landing files Task 9 also edits (walkthrough) — after 9. |
