...
- [ ] **Step 3:** `use-cascade-scroll.ts`: `useCascadeScroll(sectionRef: RefObject<HTMLElement | null>)`. Guard `prefersReducedMotion()`. `loadGsap()` → `gsap.matchMedia().add("(min-width: 1024px) and (prefers-reduced-motion: no-preference)", ...)`:
  ...
  - the cascade OWNS dock-store state for its keys while armed (measured: pinned windows sit at fixed viewport positions, so geometry triggers misbehave — `import` never spans center, review/practice flicker): drive `setActive` from scrub progress (the segment currently opening or most recently revealed; deterministic, import reachable) and `setDocked(key, revealed)` so the button pops exactly as its window opens; the useSectionDock geometry ACTIVE triggers for these keys must not run on the armed path (extend `use-section-dock.ts` minimally if needed — no other in-flight task edits it); the un-armed fallback keeps pure geometry behavior;
