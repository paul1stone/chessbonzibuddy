...
function RatingBadge({ label, value }: { label: string; value?: number }) {
  ...
  return (
    <span className="rounded bg-secondary px-1.5 py-0.5 text-[10px] text-foreground/90">
      {label}: {value}
    </span>
  );
}
  ...
  if (!isExpanded) {
    return (
      <button
        ...
        className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-left transition-colors hover:bg-secondary/60"
      >
        ...
        {hasLinkedAccount ? (
          ...
          <span className="text-xs text-muted-foreground">Link your account</span>
        )}
      </button>
    );
  }
